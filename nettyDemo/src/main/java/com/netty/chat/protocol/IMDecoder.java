...
public class IMDecoder extends ByteToMessageDecoder{
		...
		try{
			...
			if(null == content || "".equals(content.trim())){
				if(!IMP.isIMP(content)){
					//如果拿到的字符串内容不是我们的自定义协议，那么就忽略 我只解析我自己认识的协议
					ctx.channel().pipeline().remove(this);
					return;
				}
			}
			//如果我能解析了，那么就把缓冲区中的数据清掉 免得下面的解码器继续拿过去解码，然后还解不了
	...
	/**
	 * 把IMP协议字符串解析成一个IMMessage的对象，这样显得有种神秘感
	 * @param msg
	 * @return
	 */
	public IMMessage decode(String msg){
		...
		try{
			...
			long time = Long.parseLong(headers[1]); //获取命令发送时间
			String nickName = headers[2]; //获取昵称
			nickName = nickName.length() < 10?nickName:nickName.substring(0,9); //昵称最多10个字
